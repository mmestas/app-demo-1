function groupInviteToQuoteCtrl($scope, $rootScope, $mdDialog, apiSrvc, commonFnSrvc, groupInfo, userInfo, projectInfo, referenceFile, Upload, upload, envURL) {

  $scope.envURL = envURL;
  $scope.groupInfoData = groupInfo;
  console.log($scope.groupInfoData);
  $scope.consultantKeys = [];
  angular.forEach(groupInfo, function(consultant) {
    var obj = {gpKey: consultant.gpKey};
    $scope.consultantKeys.push(obj);

});
  console.log($scope.consultantKeys);
  ...
  $scope.sendGroupITQ = function(obj, file1, file2, file3) {
    obj.taskType = 1;
    ...
    upload({
      // url: __env.apiUrl+'/index.asp?remotemethodaddon=appDemo1SetProjectEngagement&RequestBinary=true&gpKey='+$scope.groupInfoData.gpKey+'&projectKey='+$scope.projectInfoData.gpKey,
      url: __env.apiUrl+'/index.asp?remotemethodaddon=appDemo1SetProjectEngagement&RequestBinary=true&consultantKeys='+consultantKeyString+'&projectKey='+$scope.projectInfoData.gpKey,
      method: 'POST',
      data : objToPost
    })
